...
import { buildCaloriesTrendPoints, buildLocalDateRange, buildProteinTrendPoints, buildSpendTrendPoints, getValidCaloriesGoal, getValidCostGoal, getValidProteinGoal, summarizeCaloriesTrend, summarizeHistoryRange, summarizeProteinTrend, summarizeSpendTrend } from '../domain/analyticsCharts'
...
export default function AnalyticsScreen() {
  ...
  return <ScreenContainer title="Analytics" subtitle="Understand your nutrition and spending over time."><div className="analytics-shell"><button className="analytics-back" type="button" onClick={openDrawer}><AnalyticsIcon name="back" />Back to More</button><div className="analytics-range-context" aria-live="polite" aria-atomic="true"><span><AnalyticsIcon name="calendar" />{rangeLabel}</span><strong>{summary.trackedDays} saved History {summary.trackedDays === 1 ? 'day' : 'days'} in this range.</strong></div><AnalyticsSummaryGrid summary={summary} /><AnalyticsChartShell trend={proteinTrend} proteinGoal={proteinGoal} caloriesTrend={caloriesTrend} caloriesGoal={caloriesGoal} spendTrend={spendTrend} costGoal={costGoal} tickLabels={trendTickLabels} rangeStart={displayDate(summary.range.startDateKey)} rangeEnd={displayDate(summary.range.endDateKey)} /><AnalyticsTotalsSection kind="live" totals={today} /><AnalyticsGoals goals={settings.macroGoals} totals={today} /><AnalyticsTotalsSection kind="planned" totals={weekly} plannedDays={plannedDays} /></div></ScreenContainer>
}
